...
import run.blog.app.theme.finders.vo.NavigationPostVo;
import run.blog.app.theme.finders.vo.PostArchiveVo;
import run.blog.app.theme.finders.vo.PostArchiveYearMonthVo;
import run.blog.app.theme.finders.vo.PostVo;
import run.blog.app.theme.router.ReactiveQueryPostPredicateResolver;
            ...
            .map(list -> {
                Map<String, List<ListedPostVo>> yearPosts = list.get()
                    .collect(Collectors.groupingBy(
                        post -> BlogUtils.getYearText(post.getSpec().getPublishTime())));
                List<PostArchiveVo> postArchives = yearPosts.entrySet().stream()
                    .map(entry -> {
                        ...
                        Map<String, List<ListedPostVo>> monthPosts = entry.getValue().stream()
                            .collect(Collectors.groupingBy(
                                post -> BlogUtils.getMonthText(post.getSpec().getPublishTime())));
