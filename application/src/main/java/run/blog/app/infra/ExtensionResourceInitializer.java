 ...
 * <p>Check whether {@link BlogProperties#getInitialExtensionLocations()} is configured
...
public class ExtensionResourceInitializer implements ApplicationListener<ApplicationStartedEvent> {
    ...
    public ExtensionResourceInitializer(BlogProperties blogProperties,
    ...
    public void onApplicationEvent(ApplicationStartedEvent initializedEvent) {
        ...
        if (!blogProperties.isRequiredExtensionDisabled()) {
            locations.addAll(REQUIRED_EXTENSION_LOCATIONS);
        }
        if (blogProperties.getInitialExtensionLocations() != null) {
            locations.addAll(blogProperties.getInitialExtensionLocations());
        }
