...
import run.blog.app.security.authentication.login.UsernamePasswordHandler;
        ...
        public Mono<Authentication> authenticate(Authentication authentication) {
            ...
            return ReactiveSecurityContextHolder.getContext()
                ...
                .<Authentication>handle((prevAuth, sink) -> {
                    ...
                    if (!(principal instanceof BlogUser blogUser)) {
                        sink.error(new TwoFactorAuthException("Invalid MFA authentication."));
                        return;
                    }
                    var encryptedSecret =
                        blogUser.getDelegate().getSpec().getTotpEncryptedSecret();
