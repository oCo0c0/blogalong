...
import run.blog.app.plugin.event.BlogPluginBeforeStopEvent;
import run.blog.app.plugin.event.BlogPluginStartedEvent;
import run.blog.app.plugin.event.BlogPluginStoppedEvent;
import run.blog.app.plugin.event.SpringPluginStartedEvent;
import run.blog.app.plugin.event.SpringPluginStoppedEvent;
import run.blog.app.plugin.event.SpringPluginStoppingEvent;
import run.blog.app.theme.DefaultTemplateNameResolver;
import run.blog.app.theme.ViewNameResolver;
import run.blog.app.theme.finders.FinderRegistry;
...
public class DefaultPluginApplicationContextFactory implements PluginApplicationContextFactory {
    ...
    public ApplicationContext create(String pluginId) {
        ...
        beanFactory.registerSingleton("blogPluginEventBridge", new BlogPluginEventBridge());
    ...
    private class BlogPluginEventBridge {
        ...
        public void onApplicationEvent(SpringPluginStartedEvent event) {
            ...
            pluginManager.getRootContext()
                .publishEvent(new BlogPluginStartedEvent(this, pluginWrapper));
        }
        ...
        public void onApplicationEvent(SpringPluginStoppingEvent event) {
            ...
            var pluginWrapper = pluginManager.getPlugin(pluginContext.getName());
            pluginManager.getRootContext()
                .publishEvent(new BlogPluginBeforeStopEvent(this, pluginWrapper));
        }
        ...
        public void onApplicationEvent(SpringPluginStoppedEvent event) {
            ...
            var pluginWrapper = pluginManager.getPlugin(pluginContext.getName());
            pluginManager.getRootContext()
                .publishEvent(new BlogPluginStoppedEvent(this, pluginWrapper));
        }
    ...
    private List<PropertySource<?>> resolvePropertySources(String pluginId,
        ...
        var blogProperties = pluginManager.getRootContext()
            .getBeanProvider(BlogProperties.class)
            .getIfAvailable();
        ...
        var configsPath = blogProperties.getWorkDir().resolve("plugins").resolve("configs");
