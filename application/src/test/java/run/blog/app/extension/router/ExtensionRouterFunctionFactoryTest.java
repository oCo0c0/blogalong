...
import run.blog.app.extension.router.ExtensionRouterFunctionFactory.GetHandler;
import run.blog.app.extension.router.ExtensionRouterFunctionFactory.ListHandler;
import run.blog.app.extension.router.ExtensionRouterFunctionFactory.UpdateHandler;
...
class ExtensionRouterFunctionFactoryTest {
    ...
    List<TestCase> testCases() {
        var listWebExchange = MockServerWebExchange.from(
            MockServerHttpRequest.get("/apis/fake.blog.run/v1alpha1/fakes").build());

        var getWebExchange = MockServerWebExchange.from(
            MockServerHttpRequest.get("/apis/fake.blog.run/v1alpha1/fakes/my-fake").build()
        );

        var createWebExchange = MockServerWebExchange.from(
            MockServerHttpRequest.post("/apis/fake.blog.run/v1alpha1/fakes").body("{}")
        );

        var updateWebExchange = MockServerWebExchange.from(
            MockServerHttpRequest.put("/apis/fake.blog.run/v1alpha1/fakes/my-fake").body("{}")
        );
