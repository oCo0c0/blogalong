...
import run.blog.app.extension.exception.SchemeNotFoundException;
...
import run.blog.app.extension.store.ReactiveExtensionStoreClient;
...
class ReactiveExtensionClientTest {
    ...
    Unstructured createUnstructured() throws JsonProcessingException {
        String extensionJson = """
            {
                "apiVersion": "fake.blog.run/v1alpha1",
                "kind": "Fake",
                "metadata": {
                    "labels": {
                        "category": "fake",
    ...
    void shouldThrowSchemeNotFoundExceptionWhenSchemeNotRegistered() {
        ...
        assertThrows(SchemeNotFoundException.class, () ->
            client.fetch(fromAPIVersionAndKind("fake.blog.run/v1alpha1", "UnRegistered"), "fake"));
    ...
    void shouldFetchAnExtension() {
        var storeName = "/registry/fake.blog.run/fakes/fake";
    ...
    void shouldCreateSuccessfully() {
        ...
        when(converter.convertTo(any())).thenReturn(
            createExtensionStore("/registry/fake.blog.run/fakes/fake"));
        when(storeClient.create(any(), any())).thenReturn(
            Mono.just(createExtensionStore("/registry/fake.blog.run/fakes/fake")));
        ...
        verify(storeClient, times(1)).create(eq("/registry/fake.blog.run/fakes/fake"), any());
    ...
    void shouldUpdateSuccessfully() {
        ...
        verify(storeClient)
            .update(eq("/registry/fake.blog.run/fakes/fake"), eq(2L), any());
    ...
    void shouldUpdateUnstructuredSuccessfully() throws JsonProcessingException {
        var fake = createUnstructured();
        var name = "/registry/fake.blog.run/fakes/fake";
        ...
        verify(storeClient)
            .update(eq("/registry/fake.blog.run/fakes/fake"), eq(12345L), any());
        verify(indexer).updateRecord(eq(updatedFake));
    }
