...
class AbstractExtensionTest {
    ...
    void groupVersionKind() {
        ...
        extension.setApiVersion("fake.blog.run/v1alpha1");
        extension.setKind("Fake");
        var gvk = extension.groupVersionKind();

        assertEquals(new GroupVersionKind("fake.blog.run", "v1alpha1", "Fake"), gvk);
    }
    ...
    void testGroupVersionKind() {
        ...
        extension.groupVersionKind(new GroupVersionKind("fake.blog.run", "v1alpha1", "Fake"));

        assertEquals("fake.blog.run/v1alpha1", extension.getApiVersion());
