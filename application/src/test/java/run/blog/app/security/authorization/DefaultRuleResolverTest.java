...
class DefaultRuleResolverTest {
    ...
    Role mockRole() {
        ...
        var rules = List.of(
            ...
            new PolicyRule.Builder().apiGroups("api.plugin.blog.run")
    ...
    List<RequestResolveCase> getRequestResolveCases() {
        return List.of(new RequestResolveCase("/api/v1/tags", "GET", false),
            ...
            new RequestResolveCase("/apis/api.plugin.blog.run/v1alpha1/plugins/foo/users", "GET",
                true),
            new RequestResolveCase("/apis/api.plugin.blog.run/v1alpha1/plugins/foo/users/bar",
                "GET", true),
            new RequestResolveCase("/apis/api.plugin.blog.run/v1alpha1/plugins/foo/posts/bar",
                "GET", false),
