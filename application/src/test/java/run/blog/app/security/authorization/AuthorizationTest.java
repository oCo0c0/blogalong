...
import run.blog.app.core.extension.Role.PolicyRule;
...
class AuthorizationTest {
    ...
    void anonymousUserAccessProtectedApi() {
        ...
        webClient.get().uri("/apis/fake.blog.run/v1/posts").exchange().expectStatus()
            .isUnauthorized();
    ...
    void anonymousUserAccessAuthenticationFreeApi() {
        ...
        webClient.get().uri("/apis/fake.blog.run/v1/posts/hello-blog").exchange()
            .expectStatus()
            .isUnauthorized();
    ...
    static class TestConfig {
        ...
        public RouterFunction<ServerResponse> postRoute() {
            return route(
                GET("/apis/fake.blog.run/v1/posts").and(accept(MediaType.APPLICATION_JSON)),
                this::queryPosts).andRoute(
                PUT("/apis/fake.blog.run/v1/posts/{name}").and(accept(MediaType.APPLICATION_JSON)),
                this::updatePost);
        }
