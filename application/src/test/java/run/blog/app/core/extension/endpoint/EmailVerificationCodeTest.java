...
class EmailVerificationCodeTest {
    ...
    void setUp() {
        ...
        var sendCodeRateLimiter = RateLimiterRegistry.of(config)
            .rateLimiter("send-email-verification-code-fake-user:hi@blog.run");
        doReturn(RateLimiterOperator.of(sendCodeRateLimiter)).when(spyUserEndpoint)
            .sendEmailVerificationCodeRateLimiter(eq("fake-user"), eq("hi@blog.run"));
    ...
    void sendEmailVerificationCode() {
        ...
        user.getSpec().setEmail("hi@blog.run");
        ...
        webClient.post()
            ...
            .bodyValue(Map.of("email", "hi@blog.run"))
            .exchange()
            .expectStatus()
            .isOk();
