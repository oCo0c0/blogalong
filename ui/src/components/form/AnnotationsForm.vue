...
import { IconArrowRight } from "@blogalong/components";

import { computed, nextTick, onMounted, ref, watch } from "vue";
import { apiClient } from "@/utils/api-client";
import type { AnnotationSetting } from "@blogalong/api-client";
...
const avaliableAnnotationSettings = computed(() => {
  return annotationSettings.value.filter((setting) => {
    if (!setting.metadata.labels?.["theme.blog.run/theme-name"]) {
      return true;
    }
    return (
      setting.metadata.labels?.["theme.blog.run/theme-name"] ===
  ...
  try {
    const { data } =
      await apiClient.extension.annotationSetting.listv1alpha1AnnotationSetting(
        {
          labelSelector: [
            `blog.run/target-ref=${[props.group, props.kind].join("/")}`,
          ],
        }
      );
