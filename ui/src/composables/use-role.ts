import type { Role } from "@blogalong/api-client";
...
export function useRoleTemplateSelection(
   ...
   *         },
   *         "annotations": {
   *           "rbac.authorization.blog.run/dependencies": "[ \"role-template-view-users\", \"role-template-change-password\" ]\n",
   *           "rbac.authorization.blog.run/module": "Users Management",
   *           "rbac.authorization.blog.run/display-name": "User manage",
   *           "rbac.authorization.blog.run/ui-permissions": "[\"system:users:manage\"]\n",
   *           "rbac.authorization.blog.run/dependency-rules": "[{\"apiGroups\":[\"\"],\"resources\":[\"users\"],\"resourceNames\":[],\"nonResourceURLs\":[],\"verbs\":[\"get\",\"list\"]},{\"apiGroups\":[\"api.blog.run\"],\"resources\":[\"users/password\"],\"resourceNames\":[],\"nonResourceURLs\":[],\"verbs\":[\"update\"]}]",
   *           "rbac.authorization.blog.run/ui-permissions-aggregated": "[\"system:users:view\"]"
   *         },
   *         "version": 9
   *       }
   *     },
   ...
   *           "rbac.authorization.blog.run/ui-permissions": "[\"system:users:view\"]\n",
