...
import type { PluginModule, RouteRecordAppend } from "@blogalong/console-shared";
...
export async function setupPluginModules(app: App) {
  ...
  try {
    const { load } = useScriptTag(
      ...
      }/apis/api.console.blog.run/v1alpha1/plugins/-/bundle.js?t=${Date.now()}`
    );
  ...
  try {
    await loadStyle(
      `${
        import.meta.env.VITE_API_URL
      }/apis/api.console.blog.run/v1alpha1/plugins/-/bundle.css?t=${Date.now()}`
    );
  } catch (e) {
