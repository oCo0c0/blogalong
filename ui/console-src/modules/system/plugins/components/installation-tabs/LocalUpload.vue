...
import { Dialog, Toast } from "@blogalong/components";
...
const endpoint = computed(() => {
  if (pluginToUpgrade.value) {
    return `/apis/api.console.blog.run/v1alpha1/plugins/${pluginToUpgrade.value.metadata.name}/upgrade`;
  }
  return "/apis/api.console.blog.run/v1alpha1/plugins/install";
});
  ...
  Dialog.success({
    ...
    onConfirm: async () => {
      try {
        ...
        await apiClient.extension.plugin.updatepluginBlogRunV1alpha1Plugin({
