...
import type { Backup } from "@blogalong/api-client";
...
import type { OperationItem } from "@blogalong/console-shared";
...
function handleDownload() {
  window.open(
    `/apis/api.console.migration.blog.run/v1alpha1/backups/${props.backup.metadata.name}/files/${props.backup.status?.filename}`,
    "_blank"
  );
}
...
function handleDelete() {
  Dialog.warning({
    ...
    async onConfirm() {
      await apiClient.extension.backup.deletemigrationBlogRunV1alpha1Backup({
