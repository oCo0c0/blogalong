...
import type { ListedSinglePage, SinglePage } from "@blogalong/api-client";
...
const {
  ...
  queryFn: async () => {
    const { data } = await apiClient.singlePage.listSinglePages({
      labelSelector: [`content.blog.run/deleted=true`],
      page: page.value,
      size: size.value,
...
const handleDeletePermanently = async (singlePage: SinglePage) => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      await apiClient.extension.singlePage.deletecontentBlogRunV1alpha1SinglePage(
        {
    ...
    onConfirm: async () => {
      await Promise.all(
        selectedPageNames.value.map((name) => {
          return apiClient.extension.singlePage.deletecontentBlogRunV1alpha1SinglePage(
            {
              name,
            }
          );
...
const handleRecovery = async (singlePage: SinglePage) => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      ...
      await apiClient.extension.singlePage.updatecontentBlogRunV1alpha1SinglePage(
        {
