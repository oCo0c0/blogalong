...
import { Toast, VButton, VModal, VSpace } from "@blogalong/components";
import SubmitButton from "@/components/button/SubmitButton.vue";
import type { Policy, PolicyTemplate } from "@blogalong/api-client";
...
const initialFormState: Policy = {
  ...
  apiVersion: "storage.blog.run/v1alpha1",
  kind: "Policy",
  metadata: {
    name: "",
...
const handleSave = async () => {
  try {
    ...
    if (isUpdateMode.value) {
      await apiClient.extension.storage.policy.updatestorageBlogRunV1alpha1Policy(
        {
      ...
      await apiClient.extension.storage.policy.createstorageBlogRunV1alpha1Policy(
        {
          policy: formState.value,
        }
      );
...
watch(
  ...
  async (policy) => {
    if (policy) {
      ...
      if (templateName) {
        const { data } =
          await apiClient.extension.storage.policyTemplate.getstorageBlogRunV1alpha1PolicyTemplate(
            {
              name: templateName,
            }
          );
