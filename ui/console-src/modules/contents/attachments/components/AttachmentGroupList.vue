...
const handleDelete = (group: Group) => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      ...
      await apiClient.extension.storage.group.deletestorageBlogRunV1alpha1Group(
      ...
      const moveToUnGroupRequests = data.items.map((attachment) => {
        ...
        return apiClient.extension.storage.attachment.updatestorageBlogRunV1alpha1Attachment(
          {
  ...
  Dialog.warning({
    ...
    onConfirm: async () => {
      ...
      const deleteAttachmentRequests = data.items.map((attachment) => {
        return apiClient.extension.storage.attachment.deletestorageBlogRunV1alpha1Attachment(
          { name: attachment.metadata.name }
        );
      });
