...
import type { Post, ListedPost } from "@blogalong/api-client";
...
const {
  ...
  queryFn: async () => {
    const labelSelector: string[] = ["content.blog.run/deleted=false"];
...
const handleOpenSettingModal = async (post: Post) => {
  const { data } = await apiClient.extension.post.getcontentBlogRunV1alpha1Post(
    {
