...
const handleDeletePermanently = async (post: Post) => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      await apiClient.extension.post.deletecontentBlogRunV1alpha1Post({
    ...
    onConfirm: async () => {
      await Promise.all(
        selectedPostNames.value.map((name) => {
          return apiClient.extension.post.deletecontentBlogRunV1alpha1Post({
            name,
          });
        })
      );
    ...
    onConfirm: async () => {
      await Promise.all(
        selectedPostNames.value.map((name) => {
          ...
          return apiClient.extension.post.updatecontentBlogRunV1alpha1Post({
            name: post.metadata.name,
            post: {
              ...post,
              spec: {
