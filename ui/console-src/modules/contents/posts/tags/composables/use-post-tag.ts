...
import type { Tag } from "@blogalong/api-client";
...
export function usePostTag(): usePostTagReturn {
  ...
  const {
    ...
    queryFn: async () => {
      const { data } =
        await apiClient.extension.tag.listcontentBlogRunV1alpha1Tag({
          page: 0,
          size: 0,
  ...
  const handleDelete = async (tag: Tag) => {
    Dialog.warning({
      ...
      onConfirm: async () => {
        try {
          await apiClient.extension.tag.deletecontentBlogRunV1alpha1Tag({
