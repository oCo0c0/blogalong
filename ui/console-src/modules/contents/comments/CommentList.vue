...
import type { ListedComment } from "@blogalong/api-client";
    ...
    onConfirm: async () => {
      try {
        const promises = selectedCommentNames.value.map((name) => {
          return apiClient.extension.comment.deletecontentBlogRunV1alpha1Comment(
            {
              name,
            }
          );
    ...
    onConfirm: async () => {
      try {
        ...
        const promises = commentsToUpdate?.map((comment) => {
          return apiClient.extension.comment.updatecontentBlogRunV1alpha1Comment(
            {
