...
import { Dialog, Toast, VDropdown, VDropdownItem } from "@blogalong/components";
import type { Theme } from "@blogalong/api-client";
...
const handleUninstall = async (deleteExtensions?: boolean) => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      try {
        await apiClient.extension.theme.deletethemeBlogRunV1alpha1Theme({
