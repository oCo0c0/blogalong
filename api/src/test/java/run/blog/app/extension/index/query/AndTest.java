...
public class AndTest {
    ...
    void testMatches() {
        ...
        Collection<Map.Entry<String, String>> ageEntry = List.of(Map.entry("19", "blog"),
            Map.entry("19", "guqing"),
        ...
        indexView = new QueryIndexViewImpl(entries);
        query = and(
            // guqing, blog, lisi, zhangsan
            or(equal("dept", "A"), equal("dept", "B")),
            // guqing, blog, zhangsan
            or(equal("age", "19"), equal("age", "18"))
        );
        resultSet = query.matches(indexView);
        assertThat(resultSet).containsExactlyInAnyOrder("guqing", "blog", "zhangsan");
